...
void Nnf_Man::init(Aig_Man_t* pSrc) {
	...
	std::cout << "Removed " << rem << " nodes during NNF cleanup" <<std::endl;
	...
	// std::cout <<"\n\nPushed Bubbles down" <<std::endl;
	// print();
	Nnf_ManTopoId();

	// std::cout <<"\n\nTopo-sorted" <<std::endl;
	// print();
}
...
Nnf_Obj* Nnf_Man::bdd2nnf_rec(DdNode* FddNode, std::map<DdNode*, Nnf_Obj*>&cache) {
...
void Nnf_Man::parse_aig(Aig_Man_t* pSrc) {
	...
	if(pSrc->pName)
		this->pName = std::string(pSrc->pName);
...
Aig_Man_t* Nnf_Man::createAig(bool withCloudInputs) {
	...
	Aig_Obj_t* pObj;
	std::vector<int> CiPosIth;
	std::vector<int> CiNegIth;
	std::vector<int> CiCloudIth;
	std::vector<int> CoIth;

	// Clear Id std::mappings
	...
	for(auto node: _inputs_pos) {
		...
		// Build InputID std::map (original -> new)
...
Aig_Man_t* Nnf_Man::createAigMultipleClouds(int numCloudSets,
	std::vector<std::vector<int>> CiCloudIth,
	std::vector<std::vector<int>> CoIth) {
	...
	for(auto node: _inputs_pos) {
		pObj = Aig_ObjCreateCi(pMan);
		CiPosIth.push_back(Aig_ObjCioId(pObj));
		node->pData = new std::vector<Aig_Obj_t*>(numCloudSets, pObj);
	...
	for(auto node: _allNodes) {
		if (Nnf_ObjIsConst1(node)) {
			node->pData = new std::vector<Aig_Obj_t*>(numCloudSets, Aig_ManConst1(pMan));
		}
		else if (Nnf_ObjIsCiPos(node)) {
			// Handled before
		}
			...
			std::vector<Aig_Obj_t*>* childVec = (std::vector<Aig_Obj_t*>*) Nnf_ObjFanin0(node)->pData;
			assert(!Nnf_ObjFaninC0(node));
			assert(childVec->size() == numCloudSets);

			node->AigNumVec.resize(numCloudSets);
			node->pData = new std::vector<Aig_Obj_t*>(numCloudSets, NULL);
			...
			for(int i = 0; i<numCloudSets; i++) {
				...
				(*(std::vector<Aig_Obj_t*>*)node->pData)[i] = pObj;
			}
		}
		else if (Nnf_ObjIsAnd(node) || Nnf_ObjIsOr(node)) {
			std::vector<Aig_Obj_t*>* childVec0 = (std::vector<Aig_Obj_t*>*) Nnf_ObjFanin0(node)->pData;
			std::vector<Aig_Obj_t*>* childVec1 = (std::vector<Aig_Obj_t*>*) Nnf_ObjFanin1(node)->pData;
			assert(!Nnf_ObjFaninC0(node));
			assert(!Nnf_ObjFaninC1(node));
	...
	for(auto node: _allNodes) {
		delete (std::vector<Aig_Obj_t*>*)node->pData;
		node->pData = NULL;
	}
...
void NNf_ObjSetFanin0(Nnf_Obj* parent, Nnf_Obj* child) {
	...
	if(childReg != NULL) {
		std::set<Nnf_Obj*>& fanoutSet = isChildC?childReg->pFanoutNeg:childReg->pFanoutPos;
...
void Nnf_Man::Nnf_ManDfs_rec(Nnf_Obj * pObj, std::vector<Nnf_Obj*> &vNodes) {
    if (pObj == NULL)
        return;
...
std::vector<Nnf_Obj*> Nnf_Man::Nnf_ManDfs() {
    std::vector<Nnf_Obj*> vNodes;
    Nnf_Obj * pObj;
    int i;
    ...
    if(vNodes.size() != _allNodes.size()) {
	    ...
	    this->print();
	    std::set<Nnf_Obj*> a(_allNodes.begin(), _allNodes.end());
	    std::set<Nnf_Obj*> v(vNodes.begin(), vNodes.end());

	    std::vector<Nnf_Obj*> diff;
	    ...
	    std::cout << "vNodes.size(): 	" << vNodes.size() <<std::endl;
	    std::cout << "_allNodes.size(): 	" << _allNodes.size() <<std::endl;
...
void Nnf_Man::Nnf_ManTopoId() {
    std::vector<Nnf_Obj*> vNodes = Nnf_ManDfs();
    int currId = 0;
    for(auto it: vNodes)
		it->Id = currId++;
    _allNodes = vNodes;
...
bool Nnf_Man::isWDNNF() {
	std::vector<int> varsY(this->getCiNum());
	iota(varsY.begin(), varsY.end(), 0);
	return this->isWDNNF(varsY);
}

bool Nnf_Man::isWDNNF(std::vector<int>& varsY) {
	// Recursive calls
	...
	// Const1 and Inputs
	this->const1()->pData = new std::set<int>();
	this->const1()->iData = 1;
	Nnf_ObjSetMarkA(this->const1());
	for (int i = 0; i < this->getCiNum(); ++i) {
		_inputs_pos[i]->pData = new std::set<int>();
		_inputs_neg[i]->pData = new std::set<int>();
	...
	for (auto i:varsY) {
		((std::set<int>*)(_inputs_pos[i]->pData))->insert(i);
		((std::set<int>*)(_inputs_neg[i]->pData))->insert(-i);
	}
...
bool Nnf_Obj::isWDNNF() {
	// Recursive calls
	// Store Support in pData as std::set<int>
	...
	switch(this->Type) {
		...
		case Nnf_Type::NNF_OBJ_CO:
			...
			this->iData = res?1:0;
			// std::cout << res << " from "; this->print();
			return res;

		case Nnf_Type::NNF_OBJ_OR:
			res = Nnf_ObjFanin0(this)->isWDNNF() and Nnf_ObjFanin1(this)->isWDNNF();
			if(res) {
				std::set<int>* s0 = (std::set<int>*) Nnf_ObjFanin0(this)->pData;
				std::set<int>* s1 = (std::set<int>*) Nnf_ObjFanin1(this)->pData;
				auto supp = new std::set<int>();
...
std::string type2String(Nnf_Type t) {
	switch(t) {
	...
	std::cerr << "Error: type2String type: " << t <<std::endl;
	assert(false);
}

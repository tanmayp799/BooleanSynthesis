...
class Nnf_Obj {
	...
	int Id;
	int AigNum;
	std::vector<int> AigNumVec;
	int OrigAigId;
	Nnf_Obj* neg;			// Stores negation-node (if available)
	Nnf_Obj* pFanin0;
	Nnf_Obj* pFanin1;
	std::set<Nnf_Obj*> pFanoutPos;
	std::set<Nnf_Obj*> pFanoutNeg;
...
class Nnf_Man {
	std::string pName;
	std::vector<Nnf_Obj*> _inputs_pos;
	std::vector<Nnf_Obj*> _inputs_neg;
	std::vector<Nnf_Obj*> _outputs;
	std::vector<Nnf_Obj*> _allNodes; //(to be stored in order of IDs) (topo-sort)
	Nnf_Obj* pConst1;
	std::map<int,int> _origToNewNodeId;
	...
	// Routines
	bool isWDNNF();
	bool isWDNNF(std::vector<int>& varsY);
	void pushBubblesDown(Nnf_Obj* nObj);
	void print();
	void Nnf_ManDfs_rec(Nnf_Obj * pObj, std::vector<Nnf_Obj*> &vNodes);
	std::vector<Nnf_Obj*> Nnf_ManDfs();
	void Nnf_ManTopoId();
	...
	Aig_Man_t* createAigMultipleClouds(int numCloudSets,
		...
		Nnf_Obj* bdd2nnf_rec(DdNode* FddNode, std::map<DdNode*, Nnf_Obj*>&cache);
	bool checkIsNnf();
